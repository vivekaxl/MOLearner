...
def process():
    ...
    rects3 = ax.bar(ind+0*width, epal_evals_1, width/2, label=r"epal\-0.01", color="#79CDCD")
    rects4 = ax.bar(ind+1*width, epal_evals_2, width/2, label=r"epal\-0.02", color="#66CCCC")
    rects5 = ax.bar(ind+2*width, epal_evals_3, width/2, label=r"epal\-0.04", color="#AEEEEE")
    rects6 = ax.bar(ind+3*width, epal_evals_4, width/2, label=r"epal\-0.08", color="#37FDFC")
    rects7 = ax.bar(ind+4*width, epal_evals_5, width/2, label=r"epal\-0.12", color="#00CDCD")
    rects8 = ax.bar(ind+5*width, epal_evals_6, width/2, label=r"epal\-0.16", color="#00FFFF")
    rects9 = ax.bar(ind+6*width, epal_evals_7, width/2, label=r"epal\-0.20", color="#E0FFFF")
    rects10 = ax.bar(ind+7*width, epal_evals_8, width/2, label=r"epal\-0.3", color="#00E5EE")
    # rects11 = ax.bar(ind+8*width, mmre_evals, width/2, color='red', label="mmre-prog")
    rects12 = ax.bar(ind+9*width, rank_evals, width/2, color='green', label="rank-prog")
    rects13 = ax.bar(ind+10*width, al_evals, width/2, color='yellow', label="AL1")
    ...
    # plt.show()
    plt.savefig('evals_al2_remove_logy_without_guo.png', bbox_inches='tight')
    print "Assa'"
