...
def scottknott(data, cohen=0.01, small=3, useA12=False, epsilon=0.01):
...
def rdivDemo(name, name2, raw_data, isLatex=False, globalMinMax=True, high=1, low=0):
    ...
    data = map(lambda lst: Num(lst[0], lst[1:]), raw_data)
    ranks = []
    ...
    # Each element in this dict is rank
    table_dict = {}
    for _, __, x in sorted(ranks):
        table_dict[x.name.replace('\\_', '')] = x.rank+1
