...
def process():
    ...
    for problem in problems:
        ...
        # lists = list()
        # lists.append(['AL1'] + problem_content['al1'][obj])
        # lists.append(['AL2'] + problem_content['al2'][obj])
        # lists.append(['MMRE'] + problem_content['mmre'][obj])
        # lists.append(['Rank'] + problem_content['rank'][obj])
        name = problem.replace('-p10000-d27-o4-dataset', '-')
        name = name.replace('-p10000-d9-o3-dataset', '-').replace('-p10000-d50-o3-dataset', '-')

        print name, ",", round(np.median(problem_content['al1'][obj]), 3), ",", round(np.median(
            problem_content['al2'][obj]), 3), ",", round(np.median(problem_content['mmre'][obj]), 3), ",", round(np.median(
            problem_content['rank'][obj]), 3)
